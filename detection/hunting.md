...
Queries are written generically; adapt field names to your SIEM. Examples are in nPL, the piped query language used by [nano](https://github.com/nano-rs/nano), which is what the lab ran.
...
## B. Process-based (RCE variants only; misses the key-dump)
...
**A detection here does not mean the attack was stopped.** Remediation ("Remove") races the payload; the OOB beacon can complete first. On any 1116/1117 with this threat, also search DNS/proxy/firewall egress for the callback domain in the surrounding ±1 minute.
